...
class Myo(object):
	...
	def address_holder(self):
		return self.addr
			

if __name__ == '__main__':
    port = "/dev/tty.usbmodem11"  # Replace with the appropriate port name
    m = Myo(port,mode=emg_mode.RAW)

def sanitize_address(address):
    sanitized_address = re.sub(r'[<>:"/\\|?*0x]', '', str(address))
    if 'Calf' in sanitized_address:
        sanitized_address = 'calf_'
    elif 'Black' in sanitized_address:
        sanitized_address = 'black_'
    elif 'White' in sanitized_address:
        sanitized_address = 'white_'
    return sanitized_address

def proc_emg(emg, moving, times=[]):
    device_name = m.address_holder()
    sanitized_address = sanitize_address(device_name)
    csv_name = sanitized_address + 'emg' + '.csv'
    EMG_DATA = pd.DataFrame(emg)  # Create a DataFrame from emg
    EMG_DATA_transposed = EMG_DATA.T
    
    timestamp = pd.to_datetime(datetime.datetime.now())
    timestamp_str = timestamp.strftime("%H:%M:%S.%f")[:-3]
    EMG_DATA_transposed['Timestamp'] = timestamp_str  
    EMG_DATA_transposed.to_csv(csv_name, mode='a', index=False, header=False)  # Append data to CSV
    print(EMG_DATA_transposed)

def proc_quat(quat):
    device_name = m.address_holder()
    sanitized_address = sanitize_address(device_name)
    csv_name = sanitized_address + 'quat' + '.csv'
    QUAT = pd.DataFrame(quat)  # Create a DataFrame from emg
    QUAT_transposed = QUAT.T
    
    timestamp = pd.to_datetime(datetime.datetime.now())
    timestamp_str = timestamp.strftime("%H:%M:%S.%f")[:-3]
    QUAT_transposed['Timestamp'] = timestamp_str
    
    QUAT_transposed.to_csv(csv_name, mode='a', index=False, header=False)  # Append data to CSV
    print(QUAT_transposed)

def proc_acc(acc):
    device_name = m.address_holder()
    sanitized_address = sanitize_address(device_name)
    csv_name = sanitized_address + 'acc' + '.csv'
    ACC = pd.DataFrame(acc)  # Create a DataFrame from emg
    ACC_transposed = ACC.T
    
    timestamp = pd.to_datetime(datetime.datetime.now())
    timestamp_str = timestamp.strftime("%H:%M:%S.%f")[:-3]
    ACC_transposed['Timestamp'] = timestamp_str
    
    ACC_transposed.to_csv(csv_name, mode='a', index=False, header=False)  # Append data to CSV
    print(ACC_transposed)

def proc_gyro(gyro):
    device_name = m.address_holder()
    sanitized_address = sanitize_address(device_name)
    csv_name = sanitized_address + 'gyro' + '.csv'
    GYRO = pd.DataFrame(gyro)  # Create a DataFrame from emg
    GYRO_transposed = GYRO.T
    
    timestamp = pd.to_datetime(datetime.datetime.now())
    timestamp_str = timestamp.strftime("%H:%M:%S.%f")[:-3]
    GYRO_transposed['Timestamp'] = timestamp_str
    
    GYRO_transposed.to_csv(csv_name, mode='a', index=False, header=False)  # Append data to CSV
    print(GYRO_transposed)
...
m.add_imu_handler(lambda quat, acc, gyro: proc_quat(quat))
m.add_imu_handler(lambda quat, acc, gyro: proc_acc(acc))
m.add_imu_handler(lambda quat, acc, gyro: proc_gyro(gyro))
	
m.sleep_mode(1)
m.set_leds([0, 0, 0], [128, 128, 255])  # purple logo and bar LEDs
m.vibrate(1)
